...
def llm_qa_search(query, top_k=5, persist_directory='chroma_db'):
    """
    Perform a Q&A search using context optimization and Instructor-based structured output.
    """
    from .llm.ollama_qa import answer_question_ollama, enhance_context_for_qa, format_context_for_prompt, parse_temporal_reference
    
    # Use the provided persist_directory for the vector store
    store = ChromaVectorStore(persist_directory=persist_directory)
    
    # Get optimized context size based on question type  
    question_type = classify_question_type(query)
    adaptive_k = get_adaptive_context_size(question_type, top_k)
    
    # Parse temporal references for filtering
    ...
    # Get documents with adaptive context sizing
    documents = []
    metadatas = []
    ...
    try:
        ...
        # If semantic search fails, continue with empty results
        enhanced_context = enhance_context_for_qa([], [], temporal_filter)
    
    # Format context for the prompt
    context = format_context_for_prompt(enhanced_context)
    
    # Get structured answer using Instructor
    qa_result = answer_question_ollama(query, context, documents, metadatas)
    ...
    # Apply the existing source filtering logic
