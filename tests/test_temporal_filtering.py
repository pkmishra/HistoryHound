...
class TestTemporalFiltering:
    ...
    def test_parse_temporal_reference_last_friday(self):
        ...
        expected_friday = expected_friday.replace(hour=0, minute=0, second=0, microsecond=0)
        
        assert start_date.date() == expected_friday.date()
        assert end_date.date() == (expected_friday + timedelta(days=1)).date()
    ...
    def test_parse_temporal_reference_today(self):
        ...
        now = datetime.now()
        expected_today = now.replace(hour=0, minute=0, second=0, microsecond=0)
        assert start_date.date() == expected_today.date()
        assert end_date.date() == (expected_today + timedelta(days=1)).date()
    ...
    def test_parse_temporal_reference_this_week(self):
        ...
        assert end_date.date() == (expected_start + timedelta(days=7)).date()
